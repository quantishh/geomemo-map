  ...
  return {
    ...
    transformIndexHtml(html) {
      let result = html
        ...
        .replace(/"name": "GeoMemo Monitor"/, `"name": "${activeMeta.siteName}"`)
        .replace(/"alternateName": "GeoMemoMonitor"/, `"alternateName": "${activeMeta.siteName.replace(' ', '')}"`)
        .replace(/"url": "https:\/\/map\.geomemo\.news\/"/, `"url": "${activeMeta.url}"`)
